...
void on_message(struct mosquitto *mosq, void *obj, const struct mosquitto_message *msg){
	...
	printf("Topic: %s\n", msg->topic);
	printf("Message: %s\n", msg->payload);
	if(strcmp(msg->payload, "y") == 0 && strcmp(msg->topic, "garaza/aktuator/tablice") == 0){
		printf("Please enter tag: ");
		scanf("%d", &tag);
		char str[2];
		sprintf(str, "%d", tag);
		mosquitto_publish(mosq, NULL, "garaza/senzor/tag", sizeof(str), str, 0, false);
	}
	else if (strcmp(msg->payload, "y") == 0 && strcmp(msg->topic, "garaza/aktuator/tag") == 0){
		printf("Tag accepted!");
	}
	else if (strcmp(msg->payload, "n") == 0 && strcmp(msg->topic, "garaza/aktuator/tag") == 0){
		printf("Invalid tag!");
	}
	//mosquitto_disconnect(mosq);
}
